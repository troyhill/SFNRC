...
sitesInBay <- sp::over(finDat.coords, bnpMod)
...
for (j in c(5, 10)) { #4:(length(names(sitesInBay) - 1))) {
  dat.l <- list()
  for (i in 13:length(unique(sitesInBay$year))) {
  ...
  ### for subregions:
  if (!exists("adat")) {
    adat    <- ldply(dat.l, function(x) t(extract(x, bnpMod, fun = mean, na.rm = TRUE)))
    adat.20 <- ldply(dat.l, function(x) t(extract(x, bnpMod, fun = function(x,...)ecdf(x)(20), na.rm = TRUE))) # proportion of total area below 20 (for salinity)
    adat.sd <- ldply(dat.l, function(x) t(raster::extract(x, bnpMod, fun = sd, na.rm = TRUE)))
  } else { ### if this isn't the first iteration, merge with existing dataframes
    adat    <- rbind(adat, ldply(dat.l, function(x) t(extract(x, bnpMod, fun = mean, na.rm = TRUE))))
    adat.20 <- rbind(adat.20, ldply(dat.l, function(x) t(extract(x, bnpMod, fun = function(x,...)ecdf(x)(20), na.rm = TRUE))))
    adat.sd <- rbind(adat.sd, ldply(dat.l, function(x) t(raster::extract(x, bnpMod, fun = sd, na.rm = TRUE))))
  }
}

### to make a wide dataset:
# names(adat)[-1] <- paste0("mean.", bnpMod@data$subRegion)
# names(adat.20)[-1] <- paste0("cellsSub20.", bnpMod@data$BOX_CODE)
# names(adat.sd)[-1] <- paste0("sd.", bnpMod@data$BOX_CODE)
...
combd$subRegion <- as.character(bnpMod@data$BOX_CODE)[as.numeric(as.character(combd$subRegion))]
...
plot(bnpMod, col = as.numeric(as.factor(bnpMod@data$BOX_CODE)))
pointLabel(coordinates(bnpMod), labels = bnpMod$BOX_CODE)
...
for (j in c(7, 12)) { #5:(length(names(sitesInBay) - 1))) {
  ...
  if (!exists("adat.seas")) {
    adat.seas    <- ldply(seas.l, function(x) t(extract(x, bnpMod, fun = mean, na.rm = TRUE)))
    adat.seas.20 <- ldply(seas.l, function(x) t(extract(x, bnpMod, fun = function(x,...)ecdf(x)(20), na.rm = TRUE))) # proportion of total area below 20 (for salinity)
    adat.seas.sd <- ldply(seas.l, function(x) t(raster::extract(x, bnpMod, fun = sd, na.rm = TRUE)))
  } else { ### if this isn't the first iteration, merge with existing dataframes
    adat.seas    <- rbind(adat.seas,    ldply(seas.l, function(x) t(extract(x, bnpMod, fun = mean, na.rm = TRUE))))
    adat.seas.20 <- rbind(adat.seas.20, ldply(seas.l, function(x) t(extract(x, bnpMod, fun = function(x,...)ecdf(x)(20), na.rm = TRUE))))
    adat.seas.sd <- rbind(adat.seas.sd, ldply(seas.l, function(x) t(raster::extract(x, bnpMod, fun = sd, na.rm = TRUE))))
  }
}
...
combd.seas$subregion <- as.character(bnpMod@data$BOX_CODE)[as.numeric(as.character(combd.seas$subregion))]
...
## For each point, find name of nearest polygon (bnpMod)
for (i in seq_along(unique(biscHyd$stn))) {
  biscHyd$subRegion[biscHyd$stn %in% unique(biscHyd$stn)[i]] <- suppressWarnings(bnpMod$BOX_CODE[which.min(gDistance(masterCoords[masterCoords$stn == unique(biscHyd$stn)[i],], bnpMod, byid=TRUE))])
  biscHyd$hectares[biscHyd$stn %in% unique(biscHyd$stn)[i]]  <- suppressWarnings(bnpMod$HECTARES[which.min(gDistance(masterCoords[masterCoords$stn == unique(biscHyd$stn)[i],], bnpMod, byid=TRUE))])
}
...
plot(biscHyd.proj, col = "cornflowerblue")
plot(bnpMod, add = TRUE)


### spatial scale: whole bay.
# a <- ddply(biscHyd[biscHyd$year %in% 1980:1989, ], .(stn, year), summarise, kacft = sum(kacft, na.rm = TRUE)) 
# ddply(a, .(stn), summarise, mean(kacft, na.rm = TRUE)) # some values differ dramatically from table on page 49 of 1995 SWIM plan summary

flow.mnth <- ddply(biscHyd, .(year, mo), summarise, m3 = sum(flow, na.rm = TRUE), m3ha = sum(m3dha, na.rm = TRUE))
flow.mnth$yrMo <- paste0(flow.mnth$year,"-", flow.mnth$mo)
flow.mnth$date <- zoo::as.yearmon(flow.mnth$yrMo, "%Y-%m")

flow.yr   <- ddply(biscHyd, .(year), summarise, m3 = sum(flow, na.rm = TRUE), m3ha = sum(m3dha, na.rm = TRUE))
flow.yr$year <- as.numeric(as.character(flow.yr$year))
flow.seas <- ddply(biscHyd, .(seas, waterYr), summarise, m3 = sum(flow, na.rm = TRUE), m3ha = sum(m3dha, na.rm = TRUE))
...
flow.mnth <- ddply(biscHyd, .(year, mo, subRegion), summarise, m3 = sum(flow, na.rm = TRUE), m3ha = sum(m3dha, na.rm = TRUE))
flow.mnth$yrMo <- paste0(flow.mnth$year,"-", flow.mnth$mo)
flow.mnth$date <- zoo::as.yearmon(flow.mnth$yrMo, "%Y-%m")

flow.yr   <- ddply(biscHyd, .(year, subRegion), summarise, m3 = sum(flow, na.rm = TRUE), m3ha = sum(m3dha, na.rm = TRUE))
flow.yr$year <- as.numeric(as.character(flow.yr$year))
flow.seas <- ddply(biscHyd, .(seas, waterYr, subRegion), summarise, m3 = sum(flow, na.rm = TRUE), m3ha = sum(m3dha, na.rm = TRUE))
flow.seas$waterYr <- as.numeric(as.character(flow.seas$waterYr))
