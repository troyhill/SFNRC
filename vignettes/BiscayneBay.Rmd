...
for (j in c(5, 10)) { #4:(length(names(sitesInBay) - 1))) {
  for (i in 1990:2010) { #1:length(unique(sitesInBay$year))) {
...
ggplot(combd, aes(y = mean, x = yr, col = subregion)) + geom_point(size  = 0.25, position = pd3) + geom_pointrange(aes(ymin = mean - sd, ymax = mean + sd), position = pd3) + theme_classic() + facet_grid(param ~ ., scales = "free_y") + ylab("") 
...
for (j in c(7, 12)) { #5:(length(names(sitesInBay) - 1))) {
  for (i in 20:length(unique(sitesInBay$seas2))) {
...
### summarize
test     <- ldply(seas.l, function(x) cellStats(x, "mean"))
test$sd  <- ldply(seas.l, function(x) cellStats(x, "sd"))[, 2]
test$param <- substr(test[, 1], 1, nchar(test[, 1]) - 8)
test$yr    <- as.numeric(substr(test[, 1], nchar(test[, 1]) - 7, nchar(test[, 1]) - 4))
test$seas  <- substr(test[, 1], nchar(test[, 1]) - 2, nchar(test[, 1]))


adat <- ldply(seas.l, function(x) t(extract(x, bnp, fun = mean, na.rm = TRUE)))
adat.20 <- ldply(seas.l, function(x) t(extract(x, bnp, fun = function(x,...)ecdf(x)(20), na.rm = TRUE))) # proportion of total area below 20 (for salinity)
adat.sd <- ldply(seas.l, function(x) t(raster::extract(x, bnp, fun = sd, na.rm = TRUE)))
# adat.sd
...
ggplot(combd.seas, aes(y = mean, x = yr, col = subregion)) + geom_point(size  = 0.25, position = pd3) + geom_pointrange(aes(ymin = mean - sd, ymax = mean + sd), position = pd3) + theme_classic() + facet_grid(param ~ seas, scales = "free_y") + ylab("") 
