...
## 1. Water quality trends in Biscayne Bay
...
agm <- ddply(fin2[, -c(2)], .(waterYr, stn, seas2), numcolwise(geoMean))
...
finalSites <- unique(sitesInBay@data$stn)

finDat.coords <- join_all(list(agm[agm$stn %in% finalSites, ], masterCoords), by = "stn")
...
targParam <- "SALINITY"
targYear  <- "2015"
biscInterp(inputData = finDat.coords[(finDat.coords@data$stn %in% sitesInBay@data$stn) & !is.na(finDat.coords@data[, targParam]), ], paramCol = targParam, year = targYear, yearCol = "waterYr")
           
# for (j in 4:length(names(finDat.coords))) {
#   for (i in 1:length(unique(finDat.coords$seas2))) {
#     targYear <- unique(finDat.coords$seas2)[i]
#     targParam <- names(finDat.coords)[j]
#     targDat   <- finDat.coords[(finDat.coords$stn %in% sitesInBay@data$stn) & !is.na(finDat.coords@data[, j]), ]
#     lims <- as.numeric(quantile(data.frame(targDat@data[, targParam]), c(0, 1), na.rm = TRUE))
#     biscInterp(inputData = targDat, paramCol = targParam, year = targYear, yearCol = "seas2", plotZLims = lims,
#                exportPlot = TRUE, plotName = paste0("/opt/physical/troy/RDATA/biscayne/output/interpolatedMaps2/", strsplit(x = targYear, "-")[[1]][2] ,"/", targParam, "_", targYear, ".png"))
#   }
# }
