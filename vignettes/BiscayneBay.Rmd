...
agm <- plyr::ddply(fin2[, -c(grep(x = names(fin2), pattern = "date|waterYr"))], plyr::.(seas2, seas, year, stn), plyr::numcolwise(geoMean))
names(agm) <- gsub(x = names(agm), pattern = " |,", replacement = "")
names(agm) <- gsub(x = names(agm), pattern = "-|[+]", replacement = ".")
...
### to make a long dataset with columns: param, year, loc, mean, sd, ecdf20
tail(melt(adat, id.vars = 1, variable.name = "subregion", value.name = "mean"))
