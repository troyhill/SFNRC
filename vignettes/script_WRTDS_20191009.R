...
WaterYearStart <- 10 # Oct



# Pull data (SFNRC Linux access) ------------------------------------------
targStns <- c("S333", "S12D", "S151", 
                # no water quality data: "S20F", "S21A", "S21", "S123", "S22", "S24", "S336", "S344", "S194", "S120", "S121",
                "S332", "S179", "S334", "S177", "S18C", "S179", "S7", "S39")

analytes <- c("PHOSPHATE, TOTAL AS P", "KJELDAHL NITROGEN, TOTAL",  "AMMONIA-N", "NITRATE+NITRITE-N", "SODIUM", "TURBIDITY", "HARDNESS AS CACO3")

dat.dfe.long <- getHydro(stns = targStns, # c(as.character(dfrInBay$stn), structs),
                         parameter_list = c("flow", "head_water", "salinity", "temperature", "tail_water",
                                            "stage", "rainfall", "precipitation", "ppt"), data_shape = "long")
dat.dfe <- getHydro(stns = targStns, # c(as.character(dfrInBay$stn), structs),
                    parameter_list = c("flow", "head_water", "salinity", "temperature", "tail_water",
                                       "stage", "rainfall", "precipitation", "ppt"), data_shape = "wide")
head(dat.dfe)


phos.dfe <- getWQ(stns = targStns, target_analytes = paste(analytes, collapse = "|"))
head(phos.dfe)
...
# wqTemp <- replaceBDLs(data = phos.dfe[(phos.dfe$param %in% targAnalyte), ])
...
#                  #wq_data = wqTemp, flow_data = dat.dfe), run.parallel = TRUE))
#                  wq_data = wqTemp[wqTemp$year >= 1989, ], flow_data = dat.dfe[dat.dfe$year >= 1989, ])))
tp <- lapply(targStns, function(stnSelect) 
  modelEstimation(convertToEgret(stn = stnSelect, target_analyte = targAnalyte, 
...
# tp <- list()
# for (i in 2:length(targStns)) {
#   tp[[i]] <- modelEstimation(convertToEgret(stn = targStns[i], target_analyte = targAnalyte, 
#                                             #wq_data = wqTemp, flow_data = dat.dfe), run.parallel = TRUE))
#                                             wq_data = wqTemp[wqTemp$stn %in% targStns[i], ], flow_data = dat.dfe[dat.dfe$stn %in% targStns[i], ]))
# }
