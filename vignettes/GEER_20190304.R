...
stn.all     <- c("COOPERTN", "FROGCITY", "G311", "GLADER", "L29C1", 
...
# get merged hydro and water quality data ---------------------------------
# analytes <- c("PHOSPHATE|NITROGEN|AMMONI|CALCIUM|POTASSIUM|SODIUM|CHLORIDE|TEMP|CONDUCTIVITY, FIELD|SILICA|MAGNESIUM|TURBIDITY|CHLOROPHYLL|SULFATE")
analytes <- c("PHOSPHATE|NITROGEN|AMMONI|CALCIUM|SODIUM|CHLORIDE|TEMP|CONDUCTIVITY, FIELD|TURBIDITY|CHLOROPHYLL|SULFATE")
analytes <- c("PHOSPHATE|CALCIUM|SODIUM|CHLORIDE|SULFATE")
allDat   <- getHydro(stns = stn.all, parameter_list = c("flow", "head_water"), getWaterQuality = TRUE,
                   target_analytes = analytes)
# allDat <- allDat[, -c(8:11)]

# summary(wqDat[wqDat$param %in% "PHOSPHATE, TOTAL AS P", ])

ggplot(allDat[allDat$flow > 0, ], aes_string(y = "`SODIUM`", x = "flow", col = "stn")) + geom_point() + theme_classic()  + facet_wrap(.~stn)
ggplot(allDat, aes_string(y = "`SODIUM`", x = "head_water", col = "stn")) + geom_point() + theme_classic() + facet_wrap(.~stn)

### quality control - need to redo this for merged getHydro output. 
...
pb <- txtProgressBar(style = 3, min = 0, max = nrow(wq.df) * (0.5 * ncol(wq.df)))
...
allDat <- allDat[c(!allDat$stn %in% "S178"), ] # remove S178 - odd effects from backflow
allDat <- seas(allDat, timeCol = "date", wetSeas = c("May", "Sep"), waterYearBegin = "Oct")
...
wq2  <- dcast(wq.df[grep(wq.df$param, pattern = analytes), ], date * stn ~ param, mean, na.rm = TRUE)
bdls <- dcast(wq.df[grep(wq.df$param, pattern = analytes), ], date * stn * param ~ bdl, mean, na.rm = TRUE)
